...
if (item) {
  ...
  console.log(activeItemOffsetTop)
...
socket.on('sms balance', function(response) {
  var balance = response.UserData.balance;
  ...
  socket.on('fetch all riders', function(response) {
    ...
    for (var key in response) {
      ...
      var riders = `<tr class="animate__animated animate__fadeIn">
            <td>${id}</td>
            <td><img class="table-icon" src="${appUrl}/assets/riders/avatars/${response[key].photo}"></td>
            <td>${name}</td>
    ...
    $('#dtable tbody').on('click', 'tr td .view-rider', function(e) {
      ...
      socket.on('fetch single rider', function(response) {
        ...
        $('.user-profile-photo').prop('src', '/assets/riders/avatars/' + response.photo);
  ...
  socket.on('fetch all drivers', function(response) {
    ...
    for (var key in response) {
      ...
      if (response[key].license === 'none' || response[key].license.length < 10) {
        ...
        var license = `<span class="badge green lighten-5 dlove green-text text-accent-2"><a href="${appUrl}/assets/drivers/documents/license/${response[key].license}">DL</a></span>`;
      }
      if (response[key].ntsa === 'none' || response[key].ntsa.length < 10) {
        ...
        var ntsa = `<span class="badge green lighten-5 nlove green-text text-accent-2"><a href="${appUrl}/assets/drivers/documents/ntsa/${response[key].ntsa}">NTSA</a></span>`;
      }
      ...
      var drvtr = `<tr class="animate__animated animate__fadeIn">
            <td>${id}</td>
            <td><img class="table-icon" src="${appUrl}/assets/drivers/avatars/${response[key].photo}"></td>
            <td>${name}</td>
            <td>${response[key].phone}</td>
    ...
    $('#dtable tbody').on('click', 'tr td .view-driver', function(e) {
      ...
      socket.on('fetch single driver', function(response) {
        ...
        console.log(response)
        for (var key in response) {
          $('.driver-photo').prop('src', appUrl + '/assets/drivers/avatars/' + response[key].photo);
          ...
          if (response[key].license === 'none' && response[key].ntsa === 'none') {
            ...
            $('.doc-count').html('1');
            $('.dlntsa').html(`<a data-sub-html="Driving License" href="${appUrl}/assets/drivers/documents/license/${response[key].license}" class="pink-link">View Driving License</a>`);
            $('.dlntsa').lightGallery();
          } else if (response[key].license === 'none' && response[key].ntsa !== 'none') {
            $('.doc-count').html('1');
            $('.dlntsa').html(`<a data-sub-html="NTSA Certificate" href="${appUrl}/assets/drivers/documents/ntsa/${response[key].ntsa}" class="pink-link">View NTSA Certificate</a>`);
            $('.dlntsa').lightGallery();
          } else if (response[key].license !== 'none' && response[key].ntsa !== 'none') {
            $('.doc-count').html('2');
            $('.dlntsa').html(`<a data-sub-html="Driving License" href="${appUrl}/assets/drivers/documents/license/${response[key].license}" class="pink-link">
                            <img src="${appUrl}/assets/drivers/documents/license/${response[key].license}" class="hidden"/>
                            View DL & NTSA Documents</a>
                            <a data-sub-html="NTSA Certificate" href="${appUrl}/assets/drivers/documents/ntsa/${response[key].ntsa}" class="pink-link">
                            <img src="${appUrl}/assets/drivers/documents/ntsa/${response[key].ntsa}" class="hidden"/>
                            </a>`);
            $('.dlntsa').lightGallery({
              thumbnail: true
            });
  ...
  socket.on('fetch all vehicles', function(response) {
    ...
    for (var key in response) {
      ...
      if (response[key].xdriver.length === 1) {
        var adriver = '<button data-id="' + vid + '" data-pid="' + package + '" class="assign-driver btn btn-sm btn-success btn-block">Assigned <i class="f7-icons size-15">pencil_ellipsis_rectangle</i><div class="ripple-container"></div></button>';
      } else {
        var adriver = '<button data-id="' + vid + '" data-pid="' + package + '" class="assign-driver btn btn-sm btn-warning btn-block">Assign <i class="f7-icons size-15">plus</i><div class="ripple-container"></div></button>';
      }
      if (package === freelance) {
        var pstatus = `<button data-id="${vid}" data-pub="${superb}" class="assign-package btn btn-sm btn-warning btn-block">Assign <i class="f7-icons size-15">plus</i><div class="ripple-container"></div></button>`;
      } else {
        var pstatus = `<button data-id="${vid}" data-pub="${superb}" class="assign-package btn btn-sm btn-success btn-block">${response[key].xpackage[0].package} <i class="f7-icons size-15">pencil_ellipsis_rectangle</i><div class="ripple-container"></div></button>`;
      }
      var id = i++;
      var allvehicles = `<tr class="animate__animated animate__fadeIn">
            <td>${id}</td>
            <td><img class="table-icon" src="${appUrl}/assets/vehicles/avatars/${response[key].photo}"></td>
    ...
    $('#dtable tbody').on('click', 'tr td .more-vehicle', function(e) {
      ...
      socket.on('super functions vehicle', function(response) {
        ...
        for (var key in response) {
          $('.owner-profile-photo').prop('src', appUrl + '/assets/vehicles/avatars/' + response[key].photo);
          ...
          if (response[key].frontphoto !== 'none') {
            var vfp1 = 1;
            $('.frontx').prop('href', appUrl + '/assets/vehicles/cars/' + response[key].frontphoto);
            $('.fronty').prop('src', appUrl + '/assets/vehicles/cars/' + response[key].frontphoto);
          } else {
            var vfp1 = 0;
            $('.frontx').remove();
          }
          if (response[key].sidephoto !== 'none') {
            var vsp1 = 1;
            $('.sidex').prop('href', appUrl + '/assets/vehicles/cars/' + response[key].sidephoto);
            $('.sidey').prop('src', appUrl + '/assets/vehicles/cars/' + response[key].sidephoto);
          } else {
            var vsp1 = 0;
            $('.sidex').remove();
          }
          ...
          if (response[key].backphoto !== 'none') {
            var vbp1 = 1;
            $('.rearx').prop('href', appUrl + '/assets/vehicles/cars/' + response[key].backphoto);
            $('.reary').prop('src', appUrl + '/assets/vehicles/cars/' + response[key].backphoto);
          } else {
            var vbp1 = 0;
            $('.rearx').remove();
          }
          if (response[key].logbook !== 'none') {
            ...
            if (exty !== 'PDF') {
              $('.magnic-logbook').html(`<a target="_blank" href="${appUrl}/assets/vehicles/logbooks/${logy}" class="primary-link magnic">View ${exty} Logbook</a>`);

            } else {
              $('.magnic-logbook').html(`<a target="_blank" href="${appUrl}/assets/vehicles/logbooks/${logy}" class="primary-link">Download ${exty} Logbook</a>`)
            }
          } else {
            var lbk = 0;
            var exty = '';
          ...
          if (response[key].insurance !== 'none') {
            ...
            if (exti !== 'PDF') {
              $('.magnic-insurance').html(`<a target="_blank" href="${appUrl}/assets/vehicles/insurance/${insy}" class="pink-link magnic">View ${exty} Insurance</a>`);
            } else {
              $('.magnic-insurance').html(`<a target="_blank" href="${appUrl}/assets/vehicles/insurance/${insy}" class="pink-link">Download ${exty} Insurance</a>`)
            }
          } else {
            var ins = 0;
            var exti = '';
