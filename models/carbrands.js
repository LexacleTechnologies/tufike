...
const Carbrand = mongoose.model('Carbrand', new mongoose.Schema({
    name: {
        index: true,
        type: String,
        required: true
...
Carbrand.find().exec(function(err, result) {
    if (err) { console.log(logSymbols.error, 'Could not initiate car brand bson import'.red) } else {
        Carbrand.createIndexes();
        if (result.length === 0) {
            var data = [{
                name: "AC"
            }, {
            ...
            Carbrand.insertMany(data).then(function() {
                console.log(logSymbols.success, 'Brands schema has been initialized successfully'.blue)
            }).catch(function(error) {
                console.log(logSymbols.error, 'Brands schema failed to initialize'.red)
            });
        }
    }
})
