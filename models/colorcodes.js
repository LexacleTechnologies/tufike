...
const Colorcode = mongoose.model('Colorcode', new mongoose.Schema({
  name: {
    index: true,
    type: String,
    required: true
...
Colorcode.createIndexes();

Colorcode.find().exec(function(err, result){
  if(err){console.log(logSymbols.error, 'Could not initiate color codes bson import'.red)}
    else {
      if(result.length === 0)
      {
        var data = [
        ...
        Colorcode.insertMany(data).then(function(){ 
          console.log(logSymbols.success, 'Colors schema has been initialized successfully'.blue)
        }).catch(function(error){ 
          console.log(logSymbols.error, 'Colors schema failed to initialize'.red)
        });
      }
    }
  })
